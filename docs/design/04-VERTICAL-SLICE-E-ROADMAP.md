...
> **A Raiz Rompida.** No centro da praça de Varreth, uma pedra fendida ao meio deixa crescer, através da própria fenda, uma raiz grossa e escura que não pertence a nenhuma árvore visível nas redondezas. Ela pulsa muito lentamente com um brilho baço e esverdeado (mais perceptível ao anoitecer), e é ao redor dela que a vila se organiza hoje. Moradores deixam pequenas oferendas discretas na base da raiz (moedas, flores secas, fitas) sem necessariamente falar sobre isso abertamente.
>
> **Origem deliberadamente não estabelecida.** Quando isso aconteceu, por que a pedra fendeu, e se a raiz "cresceu" ali ou sempre esteve ali sob a pedra — nada disso é cânone ainda. Nenhum NPC tem uma explicação definitiva, e o jogo não deve afirmar nenhuma versão como verdadeira (nem "séculos atrás", nem qualquer causa específica) até desenvolvermos esse worldbuilding deliberadamente, em conversa futura. Rumores contraditórios de NPCs são aceitáveis; uma resposta fechada, não.
...
4. **Decisão (pós-combate, só aparece depois de resolvido o perigo imediato) — duas resoluções estruturais, cada uma com nuance de diálogo que decide o delta real de Índole:**
   - **Resolução A — Entregar a pessoa a Varreth:** o jogador leva a pessoa/prova de volta à taberneira. **Reputação com Varreth sobe** sempre nesta resolução (a comunidade sabe, o jogador é visto como quem resolveu o problema) — consequência pública e visível (ex.: preço melhor na taverna, NPCs cumprimentam o jogador). A pessoa responsável é punida/expulsa (mudança de estado visível daquele NPC). **O delta de Índole não é fixo** — depende de *como* a entrega é narrada/dialogada: relatar os fatos com justiça e sem exagero pode pender para `honra`; entregar a pessoa de forma calculada para ganhar favor pessoal pode pender para `pragmatismo` ou `manipulacao`; entregá-la sem nenhuma consideração por sua situação pode pender para `crueldade`. A resolução em si não define o traço — a ação concreta escolhida dentro dela define.
   - **Resolução B — Encobrir e não entregar a pessoa:** o jogador não conta a verdade completa à taberneira — só reporta que "o perigo foi resolvido". **Reputação com Varreth não muda** (a comunidade não sabe o que realmente aconteceu). **Índole muda de acordo com o motivo concreto do encobrimento**, não automaticamente para um traço "bondoso": ajudar por empatia genuína pode pender para `compaixao`/`misericordia`; encobrir calculando que a pessoa pode ser útil depois (ou que revelar traria complicação pessoal) pode pender para `pragmatismo` ou `manipulacao`; usar a situação para obter algo em troca da pessoa pode pender para `manipulacao` sem nenhuma `compaixao` envolvida. **Reputação com aquela pessoa específica** muda conforme o que ela testemunhou o jogador fazer (`witnesses: [npcId]`), podendo render um gancho de retorno futuro — independente de qual foi o motivo real do jogador.
   - **Regra de design permanente:** nenhuma resolução é "o caminho bom" e a outra "o caminho ruim". Ambas podem gerar deltas de Índole positivos, ambíguos ou desfavoráveis conforme a ação concreta do jogador — a ambiguidade moral de Ethurel precisa aparecer aqui, não só ser declarada em texto.
5. **Consequência perceptível:** ao voltar a Varreth, o mundo reage de forma diferente conforme Reputação (pública, sempre ligada a testemunho) e Índole (interna, ligada à natureza da ação, não à resolução escolhida) — isso é o teste de aceite da quest, não apenas "a quest muda de status para concluída".

Isso cobre os itens deste pedido de revisão relacionados à quest/consequência, sem exigir mais que 1 quest, 3-4 NPCs/estados e 2-3 telas de diálogo por caminho — e sem reduzir a decisão a "bom vs. ruim".
